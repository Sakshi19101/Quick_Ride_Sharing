...
class DriverRideSimulationScreen extends StatefulWidget {
  ...
  @override
  _DriverRideSimulationScreenState createState() => _DriverRideSimulationScreenState();
}

class _DriverRideSimulationScreenState extends State<DriverRideSimulationScreen> {
  ...
  Future<void> _onRideCompleted() async {
    ...
    try {
      // Update ride and booking status to 'completed'
      await FirebaseFirestore.instance.collection('rides').doc(widget.rideId).update({'status': 'completed'});
      ...
      await FirebaseFirestore.instance.collection('drivers').doc(widget.driverId).update({'totalRides': FieldValue.increment(1)});

      Fluttertoast.showToast(
        msg: "Ride marked as completed!",
      ...
      if (mounted) {
        Navigator.pop(context); // Go back to the previous screen (My Rides)
      }
    } catch (e) {
